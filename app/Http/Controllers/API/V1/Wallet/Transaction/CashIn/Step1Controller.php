...
use App\Http\Requests\API\Wallet\Transaction\CashIn\SummaryRequest;
...
class Step1Controller extends APIBaseController
{
    ...
    public function summary(SummaryRequest $request)
    {
        try {
            ...
            $chargeAmount = 0;
            $charge = $this->calculateCommissionOrCharge($sender, $receiver, $request->amount, TransactionType::CASH_IN);
            if ($charge != false) {
                $chargeAmount = $charge['amount'];
            }

            if ( $error = (new CashInRequestValidation($sender, $receiver, $request->amount, 0, config('basic_settings.currency_text'), TransactionType::CASH_IN))->validate() )
                return $error;
