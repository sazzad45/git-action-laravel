...
use App\Http\Requests\API\Wallet\Transaction\B2BTransfer\SummaryRequest;
...
class Step1Controller extends APIBaseController
{
    ...
    public function summary(SummaryRequest $request)
    {
        try {
            ...
            $chargeAmount = 0;
            $charge = $this->calculateCommissionOrCharge($sender, $receiver, $request->amount, TransactionType::B2B_TRANSFER);
            if ($charge != false) {
                $chargeAmount = $charge['amount'];
            }

            if ($error = (new B2BTransferRequestValidation($sender, $receiver, $request->amount, 0, config('basic_settings.currency_text'), TransactionType::B2B_TRANSFER))->validate())
                return $error;
