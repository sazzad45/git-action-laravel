...
class Processor
{
    ...
    public function process()
    {
        try{
            ...
            $trxTypeId = $this->param->getTransactionType()->id;

            (new TransactionValidator(
                $senderBalanceAccount, 
                $senderAccount->id, 
                $receiverAccount->id, 
                $trxTypeId,
                collect($this->param->getDebitableAccountList())->sum('amount')
            ))->validate();
            ...
            $transaction->transaction_type_id = $trxTypeId;
            ...
            return (
                new Response(
                    ...
                    [
                        ...
                        'receiverNotificationMessage' => "You've received {$this->param->getCurrency()->name} {$this->param->getAmount()} from {$senderAccount->user->mobile_no}"
                    ]
                )
            );
